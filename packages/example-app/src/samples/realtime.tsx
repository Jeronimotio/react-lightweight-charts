...
export function RealtimeExample(props: ExampleProperties): JSX.Element {
    ...
    const series = useMemo(
        () => {
            const candles: SeriesInitialOptions<'Candlestick'> = {
                type: 'Candlestick',
                data: DATA,
                ref: callback,
            };
            // TODO: typecast problem (SeriesType vs SpecificType)
            return [candles] as SeriesInitialOptions<SeriesType>[];
        },
        [callback]
    );
    ...
    useEffect(
        () => {
            ...
            let lastClose = DATA[lastIndex].close;
            ...
            let currentBar: PartialNullable<BarData> = {
                open: null,
                high: null,
                low: null,
                close: null,
            ...
            const interval = setInterval(() => {
                ...
                candleSeries.current.update(mergeTickToBar(currentBar, noisedPrice));
...
function nextBusinessDay(time: TimePoint): TimePoint {
    const d = new Date();
...
function mergeTickToBar(currentBar: PartialNullable<BarData>, price: number): BarData {
    if (currentBar.open === null) {
        ...
        currentBar.close = price;
        currentBar.high = currentBar.high === null ? price : Math.max(currentBar.high, price);
        currentBar.low = currentBar.low === null ? price : Math.min(currentBar.low, price);
    }
    return currentBar as BarData;
}
